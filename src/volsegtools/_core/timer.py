...
class Timer:
    class Stage:
        ...
        def push_event(self, name):
            if len(self.events) != 0:
                self.events.append(Timer.Event(
                    self.events[-1].end,
                    name,
                    self
                ))
        ...
        def serialize(self, use_ms=False):
            # self.end = time.time() if self.end is None else self.end
            # print(self.start, self.end, self.end - self.start)
            self.pop_event()
            return {
                "type" : "stage",
                "time" : self.end if not use_ms else round(
                    self.total_time,
                    Timer.RESOLUTION,
                ),
                "name" : self.name,
    ...
    def serialize(use_ms=False):
        ...
        return {
            "total_time": round(
                Timer.total_time() - Timer.start,
                Timer.RESOLUTION
            ) if use_ms else Timer.total_time(),
...
class JSONTimerReporter:
    ...
    def report(self, timer_data):
        ...
        vst_logger.info("Finished writing timer report into {}".format(
            self.output_path
        ))
