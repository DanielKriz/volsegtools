...
class StdoutSizeReporter(SizeReporter):
    ...
    def report(self, channel, size_in_bytes, label):
        print("--------------------------------------------------------------")
        print("Size Evaluation of '{}_ch{} - Resolution {}.' with {}".format(
        ...
        print(("Size in bytes:      {:" + str(digit_cnt) + ".5f}B").format(
            size_in_bytes
        ))
        print(("Size in mega bytes: {:" + str(digit_cnt) + ".5f}MB").format(
            size_in_bytes / 1_000_000
        ))
        print(("Size in giga bytes: {:" + str(digit_cnt) + ".5f}GB").format(
...
class SizeEvaluationStep(PostProcessingStep):
    def __init__(
        self,
        reporter: SizeReporter = StdoutSizeReporter(),
        label: str =""
    ):
