...
class MeshConverter(Converter):
    ...
    async def transform_volume(input_path: Path) -> OpaqueDataHandle:
        ...
        logging.debug(f"Is the mesh watertight? {mesh.is_watertight}")
        x = lambda: mesh.voxelized(method='ray', pitch=0.01)
        y = lambda: mesh.voxelized(method='subdivide', pitch=0.01)
        volume = mesh.voxelized(pitch=0.01)
        xvolume = x()
        print(volume.matrix)
        print(xvolume.matrix)
        da.from_array(volume.matrix.astype(np.uint8))
