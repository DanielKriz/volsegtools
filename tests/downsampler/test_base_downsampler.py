...
def test_base_parameters_property():
    ...
    assert params.downsampling_level_bounds == original_params.downsampling_level_bounds
    assert (
        params.should_remove_original_resolution
    ...
    assert (
        params.size_per_level_bounds_in_mb
        == original_params.size_per_level_bounds_in_mb
    )


def test_base_parameters_reflect_input():
    original_params = DownsamplingParameters(should_remove_original_resolution=True)
    downsampler = BaseDownsampler(original_params)
    params = downsampler.parameters
    assert type(params) is DownsamplingParameters
    assert params.downsampling_level_bounds == original_params.downsampling_level_bounds
